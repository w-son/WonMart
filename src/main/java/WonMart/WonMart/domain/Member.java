...
public class Member {
    ...
    /*
     처음에 소셜 로그인을 실행했을 때 DB에 소셜 키가 존재하는 경우 : 회원가입을 한 적 있는 회원이므로 세션에 member_id를 저장하고 redirect
     처음에 소셜 로그인을 실행했을 때 DB에 소셜 키가 존재하지 않는 경우 : 회원가입을 진행한 후 세션에 member_id를 저장
